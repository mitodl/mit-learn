...
# Allow bypass via SKIP_VENDOR_CHECK environment variable
if [ "${SKIP_VENDOR_CHECK:-}" = "true" ]; then
    exit 0
fi
...
if [ -n "$protected_changes" ]; then
    ...
    echo ""
    echo "To bypass this check (e.g., when updating from source library), run:"
    echo "  SKIP_VENDOR_CHECK=true git commit"
    exit 1
fi
