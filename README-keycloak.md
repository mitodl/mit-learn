...
The Keycloak instance is part of the `keycloak` profile in the Composer file, so if you want to interact with it, you'll need to run `COMPOSE_PROFILES=backend,frontend,keycloak,apigateway docker compose up`. (If you start the app without the profile, you can still start Keycloak later by specifying the profile.)
...
4. Start containers with the command `COMPOSE_PROFILES=backend,frontend,keycloak,apigateway docker compose up`
