...
def _resource_vector_hits(search_result, order_by=None):
    hits = list(
        dict.fromkeys(
            readable_id
    ...
    learning_resources = LearningResource.objects.for_serialization().filter(
        readable_id__in=hits
    )
    if order_by:
        ordered_resources = learning_resources.order_by(order_by)
    else:
        """
        Always lookup learning resources by readable_id for portability
        in case we load points from external systems
        """
        resources_by_id = {r.readable_id: r for r in learning_resources}
        # Re-order to match the Qdrant ranking
        ordered_resources = [
            resources_by_id[rid] for rid in hits if rid in resources_by_id
        ]
