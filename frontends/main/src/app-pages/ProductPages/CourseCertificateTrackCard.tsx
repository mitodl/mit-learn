...
type CourseCertificateTrackCardProps = {
  course: CourseWithCourseRunsSerializerV2
}

const CourseCertificateTrackCard: React.FC<
  CourseCertificateTrackCardProps
> = ({ course }) => {
  const nextRunId = course.next_run_id
  const nextRun = course.courseruns.find((run) => run.id === nextRunId)
  const currentPrice = nextRun?.products?.[0]?.price
