...
const VideoSeriesCollectionPage: React.FC<VideoPlaylistCollectionPageProps> = ({
  playlistId,
}) => {
  ...
  return (
    <Page>
      <VideoPageHeader playlist={playlist} isSeries />
          ...
          <EpisodeListUl>
            {videos.map((video, i) => (
              <EpisodeItem
                ...
                episode={video}
                index={i + 1}
                onPlayClick={() => router.push(getVideoHref(video))}
                isPlayable
              />
            ))}
          </EpisodeListUl>
...
export default VideoSeriesCollectionPage
