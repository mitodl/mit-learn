...
export const ChatApiAxiosParamCreator = function (
  ...
  return {
    ...
    chatCreate: async (
      ...
      assertParamExists("chatCreate", "message", message)
      const localVarPath = `/api/v0/chat`
      // use dummy base URL string because the URL constructor only accepts absolute URLs.
      const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL)
      let baseOptions
      if (configuration) {
...
export const ChatApiFp = function (configuration?: Configuration) {
  ...
  return {
    ...
    async chatCreate(
      ...
      (axios?: AxiosInstance, basePath?: string) => AxiosPromise<ChatResponse>
    > {
      const localVarAxiosArgs = await localVarAxiosParamCreator.chatCreate(
        message,
        options,
      )
      const index = configuration?.serverIndex ?? 0
      const operationBasePath =
        operationServerMap["ChatApi.chatCreate"]?.[index]?.url
      return (axios, basePath) =>
        createRequestFunction(
          localVarAxiosArgs,
          globalAxios,
...
export const ChatApiFactory = function (
  ...
  return {
    /**
     * DRF view for chatbot
     * @param {ChatApiChatCreateRequest} requestParameters Request parameters.
     * @param {*} [options] Override http request option.
     * @throws {RequiredError}
     */
    chatCreate(
      requestParameters: ChatApiChatCreateRequest,
      options?: RawAxiosRequestConfig,
    ): AxiosPromise<ChatResponse> {
      return localVarFp
        .chatCreate(requestParameters.message, options)
        .then((request) => request(axios, basePath))
    },
  }
}
...
/**
 * Request parameters for chatCreate operation in ChatApi.
 * @export
 * @interface ChatApiChatCreateRequest
 */
export interface ChatApiChatCreateRequest {
  /**
   *
   * @type {string}
   * @memberof ChatApiChatCreate
   */
