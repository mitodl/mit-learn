...
def no_configurations():
    ...
    Migration 0124 seeds one configuration per field, and a
...
def test_generate_credential_metadata(
    ...
    generated = asyncio.run(generate_credential_metadata(resource, user=user))

    assert generated.fields["description"] == "A course about modelling fluid flow."
    assert generated.fields["criteria"] == [
        "Applied conservation laws",
        "Modelled fluid flow",
    ]
    # Nothing to explain when every configured field generated.
...
def test_generate_credential_metadata_records_a_failure(
    ...
    # a blank one -- and the error says which of the two it is.
    assert "criteria" not in generated.fields
    assert generated.fields["description"] == "A course about modelling fluid flow."
    assert "no criteria" in generated.errors["criteria"]
    assert "description" not in generated.errors
...
def test_generate_credential_metadata_omits_empty_values(
    ...
    # A call that succeeded and returned nothing reads differently to a caller
    # than one that failed, so it does not borrow a provider error message.
    assert generated.errors == {
        "description": "The model returned no description.",
        "criteria": "The model returned no criteria.",
    }
...
def test_generate_credential_metadata_skips_inactive_configurations(
    ...
    # Nothing was asked of the field, so there is nothing to explain either.
    assert configuration.field not in generated.fields
    assert configuration.field not in generated.errors
...
def test_get_llm_bounds_the_request(settings):
    """
    The LLM is built with a request timeout.

    Without one litellm waits 6000 seconds, long past the gateway's read
    timeout, and a hung call keeps spending after the client has given up.
    """
    settings.CREDENTIAL_METADATA_LLM_TIMEOUT = 90
    configuration = CredentialMetadataConfigurationFactory.create(
        field=CredentialMetadataField.description.name
    )
...
def test_generate_credential_metadata_truncates_a_long_error(
    ...
    """
    A provider error is trimmed to one line before it reaches the response.

    litellm quotes the upstream response body in its exceptions, which the log
    row is the place for -- see MAX_ERROR_DETAIL_CHARS.
    """
    CredentialMetadataConfigurationFactory.create(
        field=CredentialMetadataField.description.name
    )
    ...
    llm = mocker.Mock()
    llm.with_structured_output.return_value.ainvoke = mocker.AsyncMock(
    ...
    error = generated.errors["description"]
    assert len(error) == MAX_ERROR_DETAIL_CHARS + len("...")
    assert error.startswith("provider said no provider said no")
