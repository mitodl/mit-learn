...
def test_ocw_courses_etl(settings, mocker, skip_content_files):
    ...
    course_learning_materials = LearningResource.objects.filter(
        resource_type=LearningResourceType.course_learning_material.name
    )

    assert course_learning_materials.count() == (0 if skip_content_files else 1)

    if not skip_content_files:
        learning_material = course_learning_materials.first()
        assert (
        ...
        assert learning_material.course_learning_material.content_category == "Practice"
