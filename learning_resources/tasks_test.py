...
def test_import_all_mit_edx_files(settings, mocker, mocked_celery, mock_blocklist):
    ...
    with pytest.raises(mocked_celery.replace_exception_class):
        tasks.import_all_mit_edx_files.delay(
            chunk_size=4, overwrite=False, learning_resource_id=1
        )
...
def test_import_all_mitxonline_files(settings, mocker, mocked_celery, mock_blocklist):
    ...
    with pytest.raises(mocked_celery.replace_exception_class):
        tasks.import_all_mitxonline_files.delay(
            chunk_size=3, overwrite=True, learning_resource_id=None
        )
    get_content_tasks_mock.assert_called_once_with(
...
def test_import_all_xpro_files(settings, mocker, mocked_celery, mock_blocklist):
    ...
    with pytest.raises(mocked_celery.replace_exception_class):
        tasks.import_all_xpro_files.delay(chunk_size=3, learning_resource_id=1)
    get_content_tasks_mock.assert_called_once_with(
        PlatformType.xpro.name, chunk_size=3, overwrite=False, learning_resource_id=1
    )
...
@mock_aws
@pytest.mark.parametrize("with_learning_resource_id", [True, False])
def test_get_content_tasks(
    settings,
    mocker,
    mocked_celery,
    mock_xpro_learning_bucket,
    with_learning_resource_id,
):
    ...
    if with_learning_resource_id:
        learning_resource_id = courses[0].learning_resource_id
    else:
        learning_resource_id = None
