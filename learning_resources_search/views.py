        ...
        return JsonResponse(
            {
                ...
                # search endpoint honors. The penalties are score units rather
                # than the percents OpenSearch uses, since similarity scores
                # are bounded; program_boost is a fraction of a result's own
                # score (see VECTOR_SEARCH_SCORE_BOOST). score_cutoff is only
                # the absolute backstop -- the cutoff that shapes a result set
                # is relative to the query's best hit and is not overridable
                # per request (see _relative_score_floor).
