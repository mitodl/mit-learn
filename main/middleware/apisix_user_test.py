...
@pytest.mark.django_db(transaction=True)
def test_established_session_does_not_revert_email(mocker, mock_login):
    """
    A stale header must not undo an email change.

    APISIX caches userinfo for the life of its session, so after a Keycloak email
    change the header keeps serving the old address. Re-applying it would revert
    what authentication.api.sync_email_from_keycloak just wrote.
    """
    user = UserFactory.create(global_id=uuid4().hex, email="new@mit.edu")
    header = b64encode(
        json.dumps(
            {
                "sub": user.global_id,
    ...
    middleware = ApisixUserMiddleware(mocker.Mock())

    middleware.process_request(mocker.Mock(META={"HTTP_X_USERINFO": header}, user=user))
...
@pytest.mark.django_db(transaction=True)
def test_established_session_still_syncs_other_fields(mocker, mock_login):
    """Only email is held back; the rest of the header is still authoritative"""
    user = UserFactory.create(
        global_id=uuid4().hex, email="new@mit.edu", first_name="Old"
    )
    header = b64encode(
        json.dumps(
            {
...
@pytest.mark.django_db(transaction=True)
def test_new_session_applies_header_email(mocker, mock_login):
    """Without an established session the header is all we have to go on"""
    user = UserFactory.create(global_id=uuid4().hex, email="stored@mit.edu")
    header = b64encode(
        json.dumps(
            {
                "sub": user.global_id,
                "preferred_username": user.username,
                "email": "from-header@mit.edu",
                "given_name": user.first_name,
                "family_name": user.last_name,
                "name": "test user fullname",
            }
    ...
    user.refresh_from_db()
    assert user.email == "from-header@mit.edu"
