...
class BaseVersioningParser(JSONParser):
    ...
    def parse(self, stream, media_type=None, parser_context=None):
        ...
        json_data_dict = super(BaseVersioningParser, self).parse(
        ...
        if hasattr(request, "version"):
            for transform in get_transform_classes(
                self.transform_base, base_version=request.version, reverse=False
            ):
                json_data_dict = transform().forwards(
                    data=json_data_dict, request=request
                )
