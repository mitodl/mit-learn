...
def test_sloan_news_events_etl():
    """Verify that the Sloan news pipeline executes correctly"""
    with reload_mocked_pipeline(
        patch("news_events.etl.sloan_news_events.extract", autospec=True),
        patch("news_events.etl.sloan_news_events.transform", autospec=False),
        patch("news_events.etl.loaders.load_feed_sources", autospec=True),
    ) as patches:
        mock_extract, mock_transform, mock_load_sources = patches
        result = pipelines.sloan_news_events_etl()
